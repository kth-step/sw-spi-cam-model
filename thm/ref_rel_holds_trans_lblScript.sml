...
(* If spi.state = tx_trans_done, then ds_abs1 is also able to perform TX label transition *)
...
(ref_rel ds_abs1 dr spi) /\ (spi.state = tx_trans_done) ==>
...
Cases_on `dr.state` >>
rw [] >>
`(spi.state <> init_start) /\
(spi.state <> init_reset) /\
(spi.state <> init_setregs) /\
(spi.state <> spi_ready) /\
(spi.state <> tx_conf_ready) /\
(spi.state <> tx_channel_enabled) /\
(spi.state <> tx_ready_for_trans) /\
(spi.state <> tx_trans_data) /\
(spi.state <> tx_channel_disabled) /\
(spi.state <> rx_conf_ready) /\
(spi.state <> rx_channel_enabled) /\
(spi.state <> rx_receive_data) /\
(spi.state <> rx_update_RX0) /\
(spi.state <> rx_data_ready) /\
(spi.state <> rx_channel_disabled) /\
(spi.state <> xfer_conf_ready) /\
(spi.state <> xfer_channel_enabled) /\
(spi.state <> xfer_ready_for_trans) /\
(spi.state <> xfer_trans_data) /\
(spi.state <> xfer_exchange_data) /\
(spi.state <> xfer_update_RX0) /\
(spi.state <> xfer_data_ready) /\
(spi.state <> xfer_channel_disabled)` by rw [] >>
...
[`spi.state = tx_trans_done` by METIS_TAC[ABS1_TX_LBL_ENABLE_def, ref_rel_def, IS_STATE_REL_def] >>
...
rw [abs1_tx_trans_done_op_state_def, abs1_tx_trans_done_op_def,
tx_trans_done_op_state_def, tx_trans_done_op_def] >>
FULL_SIMP_TAC std_ss [ref_rel_def, ABS1_TX_LBL_ENABLE_def, IS_STATE_REL_def] >>
rw []]);
...
abs1_rx_fetch_data_op_def, abs1_rx_done_op_def, rx_receive_data_op_def] >>
FULL_SIMP_TAC std_ss [ref_rel_def, IS_STATE_REL_def] >>
rw []]);
