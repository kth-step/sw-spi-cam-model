...
(* write a value to spi.regs.CH0CONF to start the tx process, vaild when TRM = 2 *)
...
(* enable or disable the SPI channel 0 for tx and rx *)
...
tx := spi.tx with state := tx_reset_conf |>
        ...
        (spi.rx.state = rx_conf_ready) /\ (v = 1w)
then spi with <|regs := spi.regs with CH0CTRL := 1w;
rx := spi.rx with state := rx_channel_enabled |>
else if (spi.init.state = init_done) /\ (spi.tx.state = tx_idle) /\
        (spi.rx.state = rx_disable_channel)
then spi with <|regs := spi.regs with CH0CTRL := 0w;
rx := spi.rx with state := rx_reset_conf|>
else spi with err := T`

(* write the spi.regs.CH0CONF to start the rx process, vaild when TRM = 1 *)
...
case spi.rx.state of
  ...
  | rx_conf_enabled => spi with err := T
